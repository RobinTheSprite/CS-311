...
public:
    ...
    // May throw std::runtime_error
    // Strong Guarantee
    MovableCounter & operator=(const MovableCounter & rhs)
    {
        if (_copyThrow)
    ...
    //     (_assnCount has been incremented.)
    // No-Throw Guarantee
    MovableCounter & operator=(MovableCounter && rhs)
    {
        ++_assnCount;
        return *this;
    }

    //MovableCounter(MovableCounter && other) = default;
    //MovableCounter & operator=(MovableCounter && other) = delete;
...
//     Return value == false.
// Does not throw (No-Throw Guarantee)
bool operator<(const MovableCounter & a,
               const MovableCounter & b)
{ return false; }
           ...
           "[other]" )
{
    SECTION( "Count additional value-type objects used in sort" )
    {
        ...
        const size_t DATASIZE = size_t(50);  // List size
